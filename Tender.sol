contract Tender {
    ...
    function newOffer(address beneficiary, uint etherAmount, uint ID, uint delTime) returns (uint offerID) {
    ...
    function acceptOffer(uint offerNumber, address consumer, address adrsupplier) returns (uint offerID)
    {
        Offer p = offers[offerNumber];
